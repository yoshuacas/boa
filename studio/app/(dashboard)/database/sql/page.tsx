...
function saveTabs(tabs: Tab[], activeId: string) {
  try {
    // Don't persist results — they can be large and are stale after reload
...
function SqlEditorInner() {
  ...
  useEffect(() => {
    if (initialQuery) {
      // If opened with a ?query= param, open it in a new tab if tabs already exist
  ...
  // When switching tabs, push current editor content into state first, then switch
                ...
                <table className="w-full text-xs font-mono">
                  <thead>
                    <tr className="border-b border-[#2a2a2f]">
                      {result.fields.map(f => (
                        <th key={f.name} className="text-left px-3 py-1.5 text-gray-500 whitespace-nowrap">
                          {f.name}
                        </th>
                      ))}
                    </tr>
