...
export default function DatabasePage() {
  ...
  useEffect(() => {
    fetch('/api/db', {
      ...
      .then(r => r.json())
      .then(d => {
        const fetched: TableInfo[] = d.tables ?? [];
        setTables(fetched);
                ...
                <div className="flex items-center justify-between px-4 py-2 border-b border-[#2a2a2f] shrink-0">
                  ...
                  <div className="flex items-center gap-3">
                    {activeTab.schema === 'public' && (
                      <Link
                        href={`/database/${encodeURIComponent(`${activeTab.schema}.${activeTab.table}`)}`}
                    ...
                    <table className="w-full text-xs font-mono border-collapse">
                      <thead>
                        <tr className="sticky top-0 bg-[#0f1117] z-10">
                          {activeTab.result.fields.map(f => (
                            <th
                              key={f.name}
                              className="text-left px-3 py-2 text-gray-500 border-b border-[#2a2a2f] whitespace-nowrap font-medium"
                            >
                              {f.name}
                            </th>
                          ))}
                      ...
                      <tbody>
                        {activeTab.result.rows.map((row, i) => (
                          <tr
                            key={i}
                            className={`border-b border-[#1a1a1f] hover:bg-[#1a1a1f] transition-colors ${
                              i % 2 !== 0 ? 'bg-[#0f1117]' : ''
                            }`}
                          >
                            {activeTab.result!.fields.map(f => {
                              ...
                              return (
                                <td
                                  ...
                                  className={`px-3 py-1.5 whitespace-nowrap max-w-xs truncate ${
                                    truncated
                                      ? 'text-gray-300 cursor-pointer hover:text-white hover:bg-[#2a2a2f]'
                                      : 'text-gray-300'
                                  }`}
                                  onClick={
                                    truncated
                                      ? () => setExpandedCell({ col: f.name, val: display! })
                                      : undefined
                                  }
...
function CellModal({ column, value, onClose }: { column: string; value: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div
        className="bg-[#1c1c21] border border-[#2a2a2f] rounded-lg w-full max-w-lg mx-4 shadow-xl"
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-[#2a2a2f]">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={onClose} className="text-gray-600 hover:text-white transition-colors text-lg leading-none">
              ×
            </button>
          </div>
        </div>
        <pre className="px-4 py-3 text-xs font-mono text-gray-300 whitespace-pre-wrap break-all max-h-96 overflow-auto">
          {formatted}
        </pre>
      </div>
    </div>
