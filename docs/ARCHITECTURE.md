...
Api:
  ...
  Properties:
    StageName: prod
    # CORS is handled inside the Lambda against the ALLOWED_ORIGINS env
    # var rather than at the API Gateway level, so the request Origin is
    # validated against an allowlist (no wildcard) and Vary: Origin is
    # set. Configure via the AllowedOrigins CloudFormation parameter or
    # `allowedOrigins: ["https://app.example.com"]` in .boa/config.json.
```
