...
Auth works the moment your backend is deployed. Users can sign up and sign in immediately — no configuration, no email verification setup, no additional deployment steps.
...
1. Your user signs up or signs in through your frontend
2. Cognito (AWS's auth service) validates credentials and issues a session token
...
5. Your Lambda function receives the user's ID, email, and role

The pre-signup trigger auto-confirms users, so they can sign in immediately after signing up. No email verification required. (You can add verification later if your app needs it.)
...
|---------|---------|-------|
| Sign-up | Enabled | Users create their own accounts |
| Sign-in method | Email + password | Social login available with additional config |
| Email verification | Skipped | Pre-signup trigger auto-confirms users |
...
| Free tier | 10,000 MAU | No cost until you pass 10K monthly active users |
...
|------|------|---------------|
| `anon` | Request has only the API key, no user token | Whatever your access policies allow for anonymous users |
...
## Reading user info in Lambda

If you write custom Lambda functions, the BOA authorizer passes the user's info as flat keys:
...
Not by default. The pre-signup trigger auto-confirms users so they can sign in immediately. To require email verification, remove the pre-signup trigger and configure SES for sending verification emails.
...
Access tokens expire after 1 hour. Refresh tokens last 30 days. The Supabase client handles refresh automatically — your users stay signed in until they explicitly sign out or are inactive for 30 days.
