...
Web applications use the BOA Library. The BOA library is partially compatible with Supabase. It manages JWT tokens and calls the dataplane backend APIs.

The BOA library knows how to consume the OpenAPI 3.0 specification provided by the BOA Backend.

::: info STATUS
The BOA client library is not yet built. During development, `@supabase/supabase-js` is used directly as a partially compatible drop-in client.
:::
...
[pgrest-lambda](https://github.com/yoshuacas/pgrest-lambda) is the engine that makes BOA work. It is an npm package that turns a PostgreSQL database (in our case DSQL) into a PostgREST-compatible REST API with GoTrue-compatible authentication and Cedar authorization (OpenAPI 3.0). You import it into your Lambda handler; it is not a standalone service.

pgrest-lambda was built for three reasons:

1. **PostgREST is stateful.** The PostgREST project used by Supabase maintains a persistent connection pool and in-memory schema cache. It cannot run in Lambda.
2. **DSQL needs different patterns.** DSQL does not support foreign keys, so pgrest-lambda uses naming conventions to infer relationships (e.g., `customers`, `customer_id`). This enables joins without database-level constraints.
3. **DSQL does not support Row-Level Security.** Cedar replaces RLS as the authorization engine, with policies that translate to SQL WHERE clauses.
...
The BOA CLI is the single interface for the full backend lifecycle. Developers and AI agents use the same commands. Developers create a new folder for their project and start it with the `boa init` command.
...
The CLI tracks applied migrations in a `_boa_migrations` table in the DSQL database with SHA-256 checksums. Modified migrations are rejected. This prevents the silent data corruption that happens when someone edits a migration that has already been applied to production.

## Authentication: Cognito

Cognito is used to authenticate end users. The APIs exposed to the client library are not Cognito APIs directly, but the GoTrue-compatible (Supabase-compatible) endpoints provided by pgrest-lambda.
...
Both are JWTs signed with the JWT secret stored in AWS Systems Manager Parameter Store. The `role` claim determines authorization behavior.
